...
namespace pk3DS
{
	public partial class XYWE : Form
	{
		...
		/// <summary>
		/// Does not currently work for nidoran f/m, and probably not for other pokemon with special chars
		/// </summary>
		private void B_Export_Site(object sender, EventArgs e)
		{
			if (DialogResult.Yes != WinFormsUtil.Prompt(MessageBoxButtons.YesNo, "Export Encounter Areas for Site?"))
				return;

			List<EncounterArea> encounters = new();

			for (int i = 0; i < CB_LocationID.Items.Count; i++)
			{
				CB_LocationID.SelectedIndex = i;
				List<string> tdata = GetEncDataList();

				if (tdata[0].Contains("No encounters found."))
				{
					continue;
				}

				EncounterArea encounterArea = new();
				string[] idAndName = CB_LocationID.Text.Split(" - ");
				encounterArea.Id = int.Parse(idAndName[0]);
				encounterArea.Name = idAndName[1];

				// Retrieve data from each encounter type
				foreach (string encounterString in tdata)
				{
					Encounter encounter = new()
					{
						Type = Regex.Match(encounterString, @"^(.+?):").Groups[1].Value.Trim(),
						Level = int.Parse(Regex.Match(encounterString, @"Level (\d+)").Groups[1].Value.Trim())
					};
					...
					if (!encounter.Type.Contains("Horde"))
					{
						foreach (Match item in Regex.Matches(encounterString, @"(\d+)% - ([a-zA-ZÀ-ÿ0-9-’. ]+)").Cast<Match>())
						{
							...
							{
								encounter.Pokemon[name] += percentage;
							}
							else
							{
								encounter.Pokemon[name] = percentage;
